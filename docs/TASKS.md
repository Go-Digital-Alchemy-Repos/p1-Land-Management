...
| P1-AGREEMENTS | Dashboard task; independent review | SOURCE ACCEPTED, STAGING PENDING | Client onboarding (`0015`) and cancellation-review workflow (`0016`) are integrated at `08b5824`. Domain, authenticated HTTP, API/dashboard type checks, production bundle and populated dump/restore rehearsal passed. The cancellation workflow is deliberately not deployed: staging backup, exact-source release, browser role/decision acceptance and provider-reconciliation gates remain. |
