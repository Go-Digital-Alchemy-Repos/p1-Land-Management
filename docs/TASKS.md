...
| P1-PUBLIC | Public specialist | PRODUCTION VERIFIED | The current `addcdbf` production release serves all 34 public routes through the P1 gateway. Route-wide QA, 1,240px desktop frame/24px mobile-gutter checks, canonical redirects, genuine 404s, CMS fallback, WebP delivery, and security headers pass. Owner acceptance plus real-user performance and complete screen-reader evidence remain. |
...
| P1-OPS | Orchestrator | IN PROGRESS | Public and dashboard Railway services are deployed from `addcdbf` and live health checks pass. Isolated Core/CMS production initialization, shared identity, backup/restore acceptance, and owner operational acceptance remain. |
| P1-QA | Orchestrator | IN PROGRESS | 34-route production build/QA, image/layout budgets, canonical-host/slash/`.html` redirects, 404 behavior, scoped runtime/security checks, and a zero-finding production dependency audit pass. Real-user performance and complete browser/screen-reader acceptance remain. |
...
| P1-COMMERCIAL | Orchestrator + content/Core/dashboard owners | IN PROGRESS | Commercial page/intake and signed dashboard handoff are deployed; staging and bounded production delivery/readback passed. Prospect context is deployed with its guarded authorization boundary. COM-05's private assessment baseline is implemented on the integration branch and awaiting staged release verification. Verified proof, assessment-to-onboarding, target-account pursuit, attachments/reporting, and native continuity remain. COM-01–07 in [initiative](initiatives/commercial-industrial-sales.md) |
| P1-NATIVE | Native specialist; independent review | IN PROGRESS | Android build/emulator storage/account-transition checks, 52 policy/storage/sync tests, the disposable Better Auth protocol harness and SQLite outbox-storage checks passed; iOS build, physical devices, signing/distribution and controlled pilot remain. |
| P1-AGREEMENTS | Dashboard task; independent review | STAGING BASELINE, ACCEPTANCE PENDING | Client onboarding (`0015`), cancellation review (`0016`) and durable draft-only preparation (`0018`) are deployed in dashboard staging. Current web `5f0a95df` retains the queue contract and adds audit-backed service-request creation; worker `4a3911b2` passed live health/auth-boundary and worker lifecycle checks, including a synthetic preparation job without a provider-posting path. Browser role/decision acceptance, provider reconciliation and production release remain. |
