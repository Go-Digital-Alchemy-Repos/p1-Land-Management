...
## Accepted offline restart envelope

Crew work must remain usable after an app restart without connectivity, within the last verified session lifetime. The Orchestrator approved minimal SecureStore metadata containing the last server-verified crew account, origin, display label, verification timestamp and actual session expiry. This is not a grant cache. Explicit offline entry opens only that account/origin's existing encrypted downloaded snapshot and permits local capture; every network read or synchronization requires fresh identity verification.

Unknown, malformed or elapsed expiry must lock offline entry and retain protected pending work for online same-account recovery. Opening the app cannot extend validity. Known401/403 or role removal removes offline-entry metadata and detaches cached views; it never silently erases pending work. Normal sign-out removes only the current account metadata after acknowledgments. Other accounts' storage remains isolated. Required tests include offline process restart within the validity window, expiry, missing keys, wrong account, known revocation and prevention of network authorization bypass. This is accepted implementation work; device acceptance remains unproven until those tests run.
