...
## Consolidation integration — September 18, 2026

Recovered the map-only implementation from `7e056da` into the active Business Center consolidation branch. Preserved current service-area copy, images, Google Analytics CSP, and all existing routes. The separate location-sidebar and marina-image changes were not included in this scoped integration.

Fresh validation: website typecheck and production build passed (54 prerendered routes); all 50 server tests passed, including map placement, all 30 route links, coordinate bounds, and narrow tile-provider CSP; layout and navigation checks passed. Existing source-map and lazy MapLibre chunk-size warnings remain.

Current browser verification is incomplete: the in-app browser rejected both localhost and 127.0.0.1 preview URLs with `ERR_BLOCKED_BY_CLIENT`. Earlier browser evidence above belongs to the original implementation, not this integration. Production has not been updated by this integration commit. Release must preserve newer public-site work on main when packaging the exact public source; do not deploy the whole consolidation tree merely to release this map.
