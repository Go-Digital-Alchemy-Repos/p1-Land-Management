...
| Client Stack Onboarding (retired) | Historical Core operations included domain planning, DNS observation, readiness evaluation and evidence retrieval. | Retired September 20 at the Owner's direction because this is a standalone, single-client system. The Dashboard and Core routes, UI, and Dashboard-to-Core transport/OpenAPI contract are removed; historical evidence and immutable migration history remain. This is not an active Website System destination or a required migration target. |

Paths in this table are relative to the existing router mounts. Canonical new bridge paths and generated client contracts must be added together with their implementations and authorization tests. At the original inspection all five destinations were missing. Developer Resources has a consolidated destination; Client Stack Onboarding was subsequently retired rather than consolidated. Remaining active gaps are recorded in the acceptance tracker.
...
Next implementation unit: Developer Resources storage/write contract and bridge, followed by the complete native reader/editor. Keep `/admin/docs` available until parity and cross-surface editing checks pass. Then Email Templates, provider configuration ownership, and Backups. Client Stack Onboarding is retired; its historical record below does not reinstate it or waive the other acceptance requirements.

## Client Stack Onboarding — historical implementation, retired September 20, 2026

Before retirement, native `/marketing/system/onboarding` implemented the four existing Core operations using the generated dashboard API client. Explicit operation allowlisting and a live, attested Owner check protected the bridge. Existing Core evidence storage and authenticated attribution remained authoritative; no schema or copied evidence store was introduced.

The former page stayed within the dashboard shell. Saved evidence could be read independently of generating a plan. Plan generation, DNS observation and readiness evaluation appended evidence; they did not change provider DNS, hosting, or release authority. ALIAS/ANAME results explicitly required manual verification. Inputs and the last valid plan survived failures, duplicate clicks were guarded, and uncertain writes were never replayed automatically.

The prior implementation corrected two retained-admin defects: DNS requests projected only the strict schema's fields, and apex values and `www` CNAME targets were independent (so an apex IP was never reused as a CNAME hostname). Those routes are no longer available after retirement.

Historical validation included Owner/non-owner bridge and attribution tests, exact transport allowlist tests, native UI tests for evidence-only reads, strict DNS projection and uncertain writes, navigation authorization tests, API/Core/dashboard type checks and production builds. Retirement validation now requires the unavailable-route and absent-navigation checks; no synthetic production launch evidence is created.
