...
| `/admin/system/backups` | `/marketing/system/backups` | Native status/run released. An Owner-only, exact-key-confirmed database restore is on the closeout branch and passes focused tests; it has not been exercised against production. Isolated restore, media-provider recovery and prior-image rollback evidence remain required. |
