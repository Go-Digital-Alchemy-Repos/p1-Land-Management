...
Latest implementation checkpoint: on September 21, `node scripts/test-dashboard.mjs` passed all 143 isolated Dashboard/database checks and migration replay after its disposable database was correctly named `dashboard_test`; this includes Dashboard federation issuer/session handling, CMS transport grant forwarding and bounded service-key behavior. The previously recorded 58 focused Core/shared identity tests passed; shared-library/API/Core/dashboard type checks and API/Core/dashboard builds passed; Branding browser passed. An initial dashboard run failed an unrelated global client-count assertion under concurrent fixtures; it passed on rerun without an onboarding change. That isolation weakness is recorded rather than hidden.
...
| --- | --- | --- |
| Dashboard navigation and Client Stack Onboarding retirement | Historical revision `a557e8bd` was live-read-checked. A later authenticated Dashboard observation at 2026-09-21 09:38 EDT verified the former Dashboard deep link renders the SPA unavailable-page state and the sidebar has no onboarding item; exact source/deployment fields are recorded in `website-system-parity-inventory.md`. | Preserve the global `/admin` retirement gate. The Dashboard unavailable-page observation is not an HTTP-status assertion; retained Core/public route status must be checked separately before any wider retirement claim. |
| Sales/CRM parity and ownership | Native Sales presentation/settings are released; the protected three-source production preservation/replay evidence remains recorded above. | The source freeze, staff operational adoption, permanent write ownership, remaining source mappings and final cutover reconciliation require Orchestrator/Owner acceptance. See the current [CRM and `/admin` retirement cutover delta](crm-admin-retirement-cutover-delta.md). |
| CMS and Website System lifecycle | The live Website editor preview rendered without a production content write. Fresh opt-in, exact-name local PostgreSQL fixtures passed both `server/services/client-site-content-lifecycle.database.test.ts` (storage lifecycle) and `server/routes/admin/client-site-content-lifecycle.database.test.ts` (actual Express admin/public route handlers). The HTTP test persisted draft r1, allowed exactly one simultaneous r1 save as r2, published r3, served its public projection with ETag/304, restored r1 as r4 and rejected stale publication. Its authentication middleware is deliberately fixture-mocked; PostgreSQL storage, validation, route parsing and public projection are real. On September 21, `playwright.cms-acceptance.config.ts` passed 2/2 (with two cross-project skips) on a new loopback-only PostgreSQL fixture using the real Core federation consumer and a local synthetic provider: the Core-host `/api/admin/client-site-content` request returned 401 before sign-in; two authenticated Core CMS browser sessions produced the stale-save conflict; r1 was restored as r4; the restored content was republished; and the public projection plus deep-link reload were verified. Its 390px touch viewport opened the same deep link and preview, saved fixture-only content, tabbed Save draft → Publish → Save draft, and had no horizontal document overflow. | Broader CMS-family accessibility, Dashboard-host wrapper parity, and provider/object recovery remain open. The browser fixture does not write production CMS content. |
...
| Standalone public forms, validation and performance | The reviewed public-forms release was recorded at `29b292ad`; the standalone estimate service-choice correction shipped at `0377fcfa`. One labelled production QA inquiry was submitted exactly once and reached a native Sales lead plus four terminal downstream jobs; transport acceptance is recorded, while inbox receipt remains unconfirmed. | Do not repeat the controlled submission. Obtain authorized Owner confirmation of inbox delivery, then complete any remaining multi-page validation and performance acceptance without widening form effects. |
...
| Controlled public commercial inquiry | The controlled labelled QA inquiry `P1-QA-20260921-29b292ad` was submitted once; its retained receipt, native Sales lead, audit evidence and four terminal downstream jobs verify the Core-to-Dashboard handoff. | Do not resubmit. Authorized Owner inbox confirmation remains the only outstanding delivery observation; retain the resulting lead and do not bypass CAPTCHA. |
