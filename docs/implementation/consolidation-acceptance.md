...
| CMS and Website System lifecycle | The live Website editor preview rendered without a production content write. Fresh opt-in, exact-name local PostgreSQL fixtures passed both `server/services/client-site-content-lifecycle.database.test.ts` (storage lifecycle) and `server/routes/admin/client-site-content-lifecycle.database.test.ts` (actual Express admin/public route handlers). The HTTP test persisted draft r1, allowed exactly one simultaneous r1 save as r2, published r3, served its public projection with ETag/304, restored r1 as r4 and rejected stale publication. Its authentication middleware is deliberately fixture-mocked; PostgreSQL storage, validation, route parsing and public projection are real. On September 21, `playwright.cms-acceptance.config.ts` passed 1/1 on a new loopback-only PostgreSQL fixture using the real Core federation consumer and a local synthetic provider: unauthenticated access was rejected, two browser sessions produced the stale-save conflict, r1 was restored as r4, the restored content was republished, and the public projection plus deep-link reload were verified. | Mobile/accessibility acceptance, broad CMS-family parity, and provider/object recovery remain open. The browser fixture does not write production CMS content. |
