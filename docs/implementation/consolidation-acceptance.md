...
| --- | --- | --- |
| Dashboard navigation and Client Stack Onboarding retirement | Production Dashboard revision `a557e8bd` was live-read-checked: the former Client Stack Onboarding route returns `404`; the sidebar has no onboarding item; Agreements, Schedule/Recurring and grouped Marketing deep links resolve. | Preserve the global `/admin` retirement gate. This route removal does not establish that unrelated retained administration can be retired. |
...
| CMS and Website System lifecycle | The live Website editor preview rendered without a production content write. `platform/p1-core/server/services/client-site-content-lifecycle.database.test.ts` passed on a fresh, opt-in, exact-name local PostgreSQL fixture. It persisted draft r1, one of two concurrent r1 saves as r2, publish r3, restore r4 and rejected stale publication. | Full browser/API lifecycle in a disposable fixture, including accepted editor behavior, remains distinct from storage-level proof. Production CMS remains read-only unless a real approved correction is needed. Website provider/object recovery remains open. |
...
| Accounts and MFA | Read-only inventory/reconciliation evidence is retained; no profile, MFA factor, invitation or account status was changed. | Owner must disposition the seven inactive accounts and make the Owner MFA enrollment/recovery decision. Complete the controlled old-account transition after that decision. |
