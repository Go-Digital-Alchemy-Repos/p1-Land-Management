...
## Read-only capture preparation — September 19, 2026

`capture-account-access.mjs` adds a separate, unreviewed capture format. Each source uses an explicit-column projection inside a verified `REPEATABLE READ READ ONLY` transaction, with statement and lock timeouts. It excludes names, email addresses, credentials, sessions, MFA secrets and provider configuration. Captures include individual source times and SHA-256 digests; the two stores are not captured atomically. Missing profiles/access records remain null rather than inferred. Output is exclusive and mode `0600`.

Provide connection strings only through private process environment variables `P1_CAPTURE_CORE_DATABASE_URL` and `P1_CAPTURE_DASHBOARD_DATABASE_URL`; never place values in commands or reports. `P1_CAPTURE_CANONICAL_NOTIFICATIONS` records the existing routing flag, not a requested change. The CLI takes a new absolute private output path. It does not load application modules or start workers.
