...
- Client, staff and crew experiences; Better Auth is the canonical identity for the business web application and its future React Native / Expo clients, so one invited or bootstrap account works across those form factors. Node.js/TypeScript, PostgreSQL, Drizzle and Railway remain the platform. The existing copied CMS authentication remains isolated unless a separately approved federation/migration plan covers account linking, role mapping, rollout and rollback; do not silently substitute or merge identity systems.
