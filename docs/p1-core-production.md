...
## Federation error-handler lint correction — 7a116c8

On September 7, exact-source Core deployment `47db004f-df40-4877-8e2e-724443ea6dd9` reached SUCCESS on production service `45646c66-c173-4e23-81ab-64ca4d545bbe` from committed source `7a116c8bc67141fbeaa7184f098578ab789d81cf`, with image digest `sha256:6b7da62bab726d2ee432a5668e80ff3f55bfe17ed522e3cd279e3dd9873db50b`.

The federation request wrapper now normalizes a duplicate-identity database error into a separate value rather than reassigning the caught exception. This preserves the existing `409 federation_identity_conflict` behavior while satisfying the Core lint rule. Core lint completed with zero errors (20 pre-existing warnings), the full suite passed 664 tests with 27 database-gated skips, and the production build and manifest-configuration tests passed. The paired public gateway was rebuilt from the same source revision. Live Core checks returned readiness `200` with database `connected`, the protected setup status remained `needsSetup:true` and `setupTokenRequired:true`, and excluded module APIs remained unavailable. No administrator, customer data, CMS records or deployment secret was created or exposed.
