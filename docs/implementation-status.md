...
GitHub `main` commit `0b2484698b961eeaac0a00bb5fa7f8fdca7c573c` (`fix(cms): declare commercial intake contract`) is the current verified source for both Railway services: public deployment `8a6d14b6-5281-4c22-8204-107dfced44e4` and dashboard deployment `5f4743cb-1da6-480d-9e79-8ca820c84b59` each reached `SUCCESS`. The release declares both public managed forms in the generated P1 site/Core manifests (`p1-estimate` and `p1-commercial-assessment`) and public QA verifies that both generated manifests agree and that commercial-page transport uses the declared endpoint.
...
Local release validation passed the full workspace type check, a production dependency audit with zero known vulnerabilities, the public production build and 34-route SSR/CMS/metadata/accessibility-structure QA, commercial intake tests, image loading, FAQ schema, and shared-frame checks. The largest route initial JavaScript remained 132.5 KiB gzip.
