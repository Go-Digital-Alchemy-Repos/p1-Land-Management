...
**Implementation status — September 7, 2026:** source migration `0018_agreement_preparation.sql`, the durable scanner, worker claim fencing, local draft preparation, financial retry controls, and office discovery endpoints are implemented and deployed to isolated staging. The isolated agreement suite replays the migration and verifies fixed and reviewed-visit preparation, stable deduplication, staff-versus-worker attribution, retry after an eligibility change, and draft-only output. A deployed worker rehearsal created one synthetic draft with worker attribution and removed the fixture. This capability has not been deployed to production; recovery, operator-flow, provider, and pilot acceptance remain required before release approval.
