...
Two-factor authentication is optional by default. An owner acts as the super admin and can require or remove MFA for any active staff account in dashboard Settings. The Better Auth TOTP flow requires the current password to enable or disable it; recovery codes are displayed at enrollment. `session_assurance` records successful TOTP/recovery-code verification for audit and enforces any super-admin MFA requirement for the current session. No setup-reset shortcut exists.
