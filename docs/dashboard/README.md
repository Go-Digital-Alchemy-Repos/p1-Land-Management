...
Calendar follow-up: deployed day/week view includes New York navigation, assignment filters, unscheduled work and accessible job selection. Date helper tests cover midnight, DST, leap year and year boundaries. Browser fixture verified selection, day mode and unassigned filtering. Deployed frontend checkpoint7e42834 on backend4079996; this view does not yet provide schedule editing or capacity validation, and the existing work-order response remains capped at500 records.

Scheduling follow-up (local, pending deployment): date-range cursor loading and versioned rescheduling/assignment controls are implemented. Crew capacity/duration conflicts and staff availability management remain outstanding.

Property photos follow-up (local, pending deployment): history includes a protected photo gallery, full-image links, publication status for managers and explicit per-photo publishing using the existing reviewed-work guard. Browser fixture verified failed-image feedback and a rejected publication staying private. Real authenticated S3 image delivery and full selective report publication acceptance remain outstanding.
