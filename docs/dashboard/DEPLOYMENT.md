...
Reviewed dashboard source `ed20808` adds additive migration `0018_agreement_preparation`, bounded source scanning, fenced preparation jobs and staff-retry records. It only creates internal `billing_draft` records after the existing agreement eligibility and approved-estimate checks; it does not post to QuickBooks, send an invoice, charge a customer or publish work. Local API type checking, the dashboard suite, and the disposable PostgreSQL migration/replay agreement run passed, including the new durable-preparation regression. The production dependency audit reported zero findings.

Staging web `803ded4d-b2bf-4e26-93e8-88804c724208` reached SUCCESS. `/api/healthz` returned `200` with `no-store`; anonymous `/api/v1/agreement-preparation-jobs` returned `401` with `no-store`, confirming the new route is mounted but not public. Matching isolated worker `4a3911b2-9f0d-4d5f-94c5-5a88513367ed` reached SUCCESS. Its startup logs recorded `worker.started`, `agreement_preparation.scheduled` and `agreement_preparation.completed`; the logs contain no payload, provider call or customer information.

The first source-repository deployment attempts exposed a Railway execution detail: pre-deploy commands run outside the Docker image and cannot address the image's compiled `dist` files. The staging service now runs `pnpm --dir artifacts/api-server migrate:dashboard` as its pre-deploy gate; the application image retains its ordinary `node dist/dashboard/main.js` start command. This is a staging-only configuration correction. Production, Core, public marketing and provider configuration were not changed. Browser role/retry acceptance, provider reconciliation and production release remain separate gates.
