...
function AuditRow({
  ...
  const routes = useContext(AuditRoutes);
  editPath = routes.edit(type, item) || "";
  previewPath = routes.preview(type, item);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 flex-shrink-0">
        ...
        {editPath && (
          <a href={editPath} className="seo-edit-link" data-testid={`audit-edit-${item.id}`}>
            {routes.editLabel?.(type) || "Edit"}
          </a>
        )}
      </div>
    </div>
...
type AuditKind = "page" | "post" | "event";
export interface AuditRouteAdapter {
  edit: (kind: AuditKind, item: AuditItem) => string | null;
  preview: (kind: AuditKind, item: AuditItem) => string | undefined;
  editLabel?: (kind: AuditKind) => string;
}
const AuditRoutes = createContext<AuditRouteAdapter>({
  edit: () => null,
  preview: () => undefined,
});
export function SeoAuditPresentation({
  data,
  isLoading,
  error,
  routes,
  ...
  isLoading: boolean;
  error?: unknown;
  routes: AuditRouteAdapter;
}) {
  return (
    <AuditRoutes.Provider value={routes}>
      <div className="seo-presentation">
        <AuditContent data={data} isLoading={isLoading} error={error} />
      </div>
    </AuditRoutes.Provider>
  );
}
