...
export function CompanyInformationPresentation({
  branding,
  ui: { Card, CardContent },
  social,
  emptyMessage = "Global — serving Core Platforms worldwide",
  titleClassName,
  bodyClassName,
  linkClassName,
}: {
  ...
  ui: { Card: ComponentType<any>; CardContent: ComponentType<any> };
  social?: ReactNode;
  emptyMessage?: string;
          ...
          <div className="space-y-2">
            <div>
              {hasCompanyInfo ? (
                ...
                <p className={cn("text-sm text-muted-foreground", bodyClassName)}>{emptyMessage}</p>
              )}
            </div>
