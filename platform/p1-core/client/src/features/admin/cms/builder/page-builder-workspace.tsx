...
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "./builder-host";
import { Badge } from "./builder-host";
import { Bookmark, ListOrdered, Monitor, Plus, Settings2, Sparkles } from "lucide-react";
import { type BlockInstance, type BuilderContent } from "./block-registry";
import { createFallbackBlockDef } from "../../../../../../shared/cms-builder/fallback-block";
import { FrontendPreviewDialog, useBuilderHost } from "./builder-host";
...
export function PageBuilderWorkspace({ content, onChange }: PageBuilderProps) {
  const host = useBuilderHost();
  const { getBlockDef, createBlock, blocks: ALL_BLOCKS } = host;
  ...
  }, [blocks, navigatorSearch, getBlockDef]);
  ...
  const addBlockAtIndex = useCallback(
    ...
    [blocks, selectBlock, setBlocks, createBlock],
  );
  ...
  const moveBlockToIndex = useCallback(
    (sourceId: string, targetIndex: number) => {
      ...
      const adjustedTargetIndex = sourceIndex < targetIndex ? targetIndex - 1 : targetIndex;
      nextBlocks.splice(
        Math.max(0, Math.min(nextBlocks.length, adjustedTargetIndex)),
        0,
        movedBlock,
      );
      setBlocks(nextBlocks);
  ...
  const handleDragOver = useCallback(
    (event: DragEvent, targetId: string) => {
      const hasExternalInsertPayload = Array.from(event.dataTransfer.types).includes(
        "application/x-page-builder-insert",
      );
      ...
      event.dataTransfer.dropEffect =
        draggedInsertPayload || hasExternalInsertPayload ? "copy" : "move";
  ...
  const handleCanvasDropZoneDragOver = useCallback(
    (event: DragEvent, index: number) => {
      const hasExternalInsertPayload = Array.from(event.dataTransfer.types).includes(
        "application/x-page-builder-insert",
      );
      const hasPayload =
        Boolean(draggedBlockId || draggedInsertPayload) || hasExternalInsertPayload;
  ...
  const handleCanvasDropZoneDrop = useCallback(
    (event: DragEvent, index: number) => {
      event.preventDefault();
  ...
  const inspectorPanel = (
    <BlockInspectorPanel
      ...
      onSaveSection={() =>
        host.canUseSections && selectedBlock && setSavingSectionBlockId(selectedBlock.id)
      }
