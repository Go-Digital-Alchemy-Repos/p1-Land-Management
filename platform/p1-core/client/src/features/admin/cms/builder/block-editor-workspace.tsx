...
function ArrayItemsField({
  ...
  const { forms, pages, galleries } = useBuilderHost().useCatalog({
    forms: schema.some((field) => field.type === "form-select"),
    pages: schema.some((field) => field.type === "url" && isButtonLinkFieldKey(field.key)),
  });
              ...
              <div key={field.key} className="space-y-1">
                ...
                {field.type === "boolean" ? (
                  ...
                  <CmsImageUpload
                    ...
                    onChange={(url: string) => updateItem(idx, field.key, url)}
                    data-testid={`array-item-${idx}-${field.key}`}
                  />
                ) : field.type === "textarea" ? (
                  shouldUseRichTextEditor(field) ? (
                    <CmsRichTextEditor
                      value={String(item[field.key] ?? "")}
                      onChange={(val: string) => updateItem(idx, field.key, val)}
...
function PropField({
  ...
  const { forms, pages, galleries } = useBuilderHost().useCatalog({
    forms: propDef.type === "form-select",
    pages: propDef.type === "url" && isButtonLinkFieldKey(propDef.key),
    galleries: propDef.type === "gallery-select",
  });
        ...
        <div key={propDef.key}>
          ...
          <div className="space-y-1.5">
            ...
            <PropField
              ...
              values={props}
              onChange={(val: unknown) => setProp(propDef.key, val)}
            />
          </div>
