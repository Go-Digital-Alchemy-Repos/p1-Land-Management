import {
  ...
  MediaDetailsContent,
  MediaUploadContent,
} from "@/components/shared/media-library-presentation";
...
import { Search, Trash2, Copy, Upload, Save, Crop, Loader2 } from "lucide-react";
  ...
  return (
    <AdminSidebar>
      <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
        <MediaLibraryGallery
          assets={assets}
          visible={filteredAssets}
          loading={isLoading}
          filtered={Boolean(search || usageFilter !== "all" || typeFilter !== "all")}
          onOpen={(id) => setSelectedAsset(assets.find((asset) => asset.id === id) ?? null)}
          uploadButton={
            <Button
      ...
      <Dialog open={uploadOpen} onOpenChange={setUploadOpen}>
        <DialogContent className="max-w-md">
          <MediaUploadContent Title={DialogTitle} Description={DialogDescription}>
            <CmsImageUpload
        ...
        {selectedAsset && (
          <DialogContent className="w-[min(96vw,72rem)] max-w-5xl h-[min(94vh,920px)] overflow-hidden p-0 gap-0 flex flex-col">
            <MediaDetailsContent
              ...
              renderField={(field) => {
                const Field = field.rows ? Textarea : Input;
                return (
                  <Field
                    id={field.id}
