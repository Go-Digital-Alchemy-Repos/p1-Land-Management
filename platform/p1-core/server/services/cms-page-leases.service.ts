...
import { db } from "../db";
import { cmsPages, editorLocks, type User, type EditorLock } from "@shared/schema";
import { CmsMutationError, databaseNow, lockPageResource } from "./cms-concurrency";
...
export async function pageLease(
  ...
  if (!user || (user.role !== "admin" && user.role !== "editor"))
    throw new CmsMutationError(400, "CMS_PAGE_LEASE_LOST", "Sign in again.");
  ...
  return db.transaction(async (tx) => {
    ...
    await lockPageResource(tx, page.id);
    // Deletion also uses the resource lock; recheck after acquiring it.
    const [exists] = await tx
      .select({ id: cmsPages.id })
      .from(cmsPages)
      .where(eq(cmsPages.id, page.id));
    if (!exists) throw new CmsMutationError(404, "CMS_PAGE_NOT_FOUND", "Page not found");
    const rows = await tx
      .select()
      .from(editorLocks)
