...
function hydrateMissingCommercialServiceOptions(fields: CmsFormField[]): CmsFormField[] {
  const services = fields.find((field) => field.key === "services");
  if (!services || services.type !== "checkbox" || services.options.length > 0) return fields;

  return fields.map((field) =>
    field.key === "services" ? { ...field, options: COMMERCIAL_SERVICE_OPTIONS } : field,
  );
}
