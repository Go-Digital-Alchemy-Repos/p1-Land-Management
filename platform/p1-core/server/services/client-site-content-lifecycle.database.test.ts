...
const fixture = vi.hoisted(() => ({ url: process.env.CLIENT_SITE_CONTENT_TEST_DATABASE_URL }));
...
suite("Client Site Content lifecycle on isolated PostgreSQL", () => {
  beforeAll(async () => {
    ...
    if (
      ...
      throw Error("Dedicated loopback named fixture database required");
    await migrate(db, { migrationsFolder: "p1-migrations" });
  }, 30000);

  beforeEach(async () => {
    await pool.query(
      "TRUNCATE client_site_content_revisions,client_site_content,blog_static_import_receipts,blog_publication_routes,blog_post_revisions,blog_publication_state,blog_posts,editor_locks,users CASCADE",
    );
