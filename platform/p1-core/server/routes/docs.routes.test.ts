...
vi.mock("../middleware/auth",()=>({authenticateToken:(req:any,res:any,next:any)=>req.get("x-auth")==="false"?res.sendStatus(401):next(),requireRole:()=> (req:any,res:any,next:any)=>req.get("x-role")==="staff"?res.sendStatus(403):next()}));
vi.mock("../storage/docs.storage",()=>({docVersion:()=>"a".repeat(64)}));
...
it("keeps legacy authentication and role gates on every mutation",async()=>{
 for(const [headers,status] of [[{"x-auth":"false"},401],[{"x-role":"staff"},403]] as const)
 ...
 expect((await request(path,method,body,headers)).status).toBe(status);
 expect(state.save).not.toHaveBeenCalled();expect(state.sync).not.toHaveBeenCalled();
});
