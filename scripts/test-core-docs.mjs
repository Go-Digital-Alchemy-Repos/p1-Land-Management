...
const name = "p1-docs-test-" + randomUUID().slice(0, 8);
...
try {
  ...
  if (!ready) throw Error("Synthetic document database did not start");
  ...
  const env = {
    ...process.env,
    DOCS_TEST_DATABASE_URL: `postgresql://postgres:${password}@127.0.0.1:${port}/core_docs_test`,
  };
  const result = await exec(
    "npx",
    ["vitest", "run", "server/__tests__/docs-atomic.db.test.ts"],
    {
  ...
  console.error("Disposable document validation failed.");
  process.exitCode = 1;
} finally {
