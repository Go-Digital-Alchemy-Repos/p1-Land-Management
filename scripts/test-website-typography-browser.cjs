...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"});try{
 ...
 await page.goto("http://127.0.0.1:4347/marketing/design/typography");await page.getByRole("alert").filter({hasText:"Could not load"}).waitFor();assert.equal(await page.getByRole("button",{name:"Save website fonts"}).count(),0);
 const reload=page.getByRole("button",{name:"Reload saved fonts",exact:true});await reload.click();
 const body=page.getByLabel("Body font",{exact:true}),heading=page.getByLabel("Heading font",{exact:true}),save=page.getByRole("button",{name:"Save website fonts",exact:true});
 await body.waitFor();assert.equal(await body.inputValue(),"legacy-font");await heading.selectOption("lora");await save.click();await page.getByRole("alert").filter({hasText:"could not be confirmed"}).waitFor();assert.equal(await heading.inputValue(),"lora");assert(await save.isDisabled());assert.deepEqual(writes[0],{fonts:{frontend_heading_font:"lora"},expectedVersion:"a".repeat(64)});
 ...
 allowed=false;await page.reload();await page.getByRole("region",{name:"Website typography"}).waitFor({state:"hidden"});assert.deepEqual(errors,[]);console.log("Typography browser passed: specific grant, custom preservation, partial writes, stale/failed reload/lost response recovery, defaults and mobile.");
