...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'});try{
 ...
 const reload=page.getByRole('button',{name:'Reload saved branding settings',exact:true});await reload.click();const name=page.getByLabel('Business Name',{exact:true}),logo=page.getByLabel('Website logo URL',{exact:true}),save=page.getByRole('button',{name:'Save Branding Settings',exact:true});
 ...
 discard=true;failRead=true;await reload.click();await page.getByRole('alert').filter({hasText:'Could not load'}).waitFor();assert.equal(await name.inputValue(),'New name');await reload.click();await page.waitForFunction(()=>document.querySelector('#company-name')?.value==='Old name');
 await name.fill('New name');lost=true;await save.click();await page.getByRole('alert').filter({hasText:'could not be confirmed'}).waitFor();await reload.click();await page.waitForFunction(()=>document.querySelector('#company-name')?.value==='New name');assert.deepEqual(writes.at(-1).settings,{company_name:'New name'});assert.equal(await logo.inputValue(),'legacy relative logo');
 await page.locator('.branding-image-card').first().locator('summary').click();await logo.fill('javascript:bad()');assert(await save.isDisabled());await logo.fill('');await save.click();await page.getByRole('status').filter({hasText:'Branding settings saved'}).waitFor();assert.deepEqual(writes.at(-1).settings,{frontend_logo_url:''});
