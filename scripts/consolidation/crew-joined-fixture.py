...
def control(action,directory):
 ...
 if action in ('verify','offline','online'):
  req=urllib.request.Request(state['origin']+('/__fixture/verify' if action=='verify' else '/__fixture/network'),data=(b'{}' if action=='verify' else json.dumps({'offline':action=='offline'}).encode()),headers={'Content-Type':'application/json','X-Fixture-Control':state['control']},method='POST')
...
if __name__=='__main__':
 parser=argparse.ArgumentParser();parser.add_argument('action',choices=['run','verify','stop','offline','online']);parser.add_argument('directory',nargs='?');args=parser.parse_args()
