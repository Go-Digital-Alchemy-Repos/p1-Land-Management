...
(async()=>{
 ...
 try{
  ...
  assert(await page.getByRole("button",{name:"Save Color Palette"}).isDisabled());
  const reload=page.getByRole("button",{name:"Reload saved colors",exact:true});await reload.click();
  const primary=page.getByLabel("Primary Color",{exact:true}),body=page.getByLabel("Paragraph Text",{exact:true}),save=page.getByRole("button",{name:"Save Color Palette",exact:true});
  ...
  assert.equal(await page.locator(".color-preview-h1").evaluate(e=>getComputedStyle(e).color),"rgb(171, 205, 239)");
  await page.screenshot({path:"/tmp/p1-website-colors-mobile.png"});
  allowed=false;await page.reload();await page.getByRole("region",{name:"Website color palette",exact:true}).waitFor({state:"hidden"});
