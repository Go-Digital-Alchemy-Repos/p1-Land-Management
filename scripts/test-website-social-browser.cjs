...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"});try{
 ...
 await page.goto('http://127.0.0.1:4347/marketing/design/social-media');await page.getByRole('alert').filter({hasText:'Could not load'}).waitFor();assert(await page.getByRole('button',{name:'Save Social Media'}).isDisabled());
 const reload=page.getByRole('button',{name:'Reload saved social settings',exact:true});await reload.click();
 const facebook=page.getByLabel('Facebook',{exact:true}),style=page.getByLabel('Icon Style',{exact:true}),save=page.getByRole('button',{name:'Save Social Media',exact:true}),preview=page.getByRole('region',{name:'Social icon preview'});
 ...
 const beforePreview=writes.length;for(const name of names)await page.locator("#social-"+name).fill("https://example.test/"+name);assert.equal(await preview.getByRole("link").count(),10);assert.equal(await preview.locator("svg").count(),10);assert.equal(writes.length,beforePreview);
