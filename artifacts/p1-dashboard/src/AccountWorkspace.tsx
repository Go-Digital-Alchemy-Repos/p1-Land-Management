...
export function ClientWorkspace({ id, tab, request, onTab, onProperty }: WorkspaceProps) {
  ...
  return <article className="account-workspace atlas-clients">
    <section className="page-hero account-hero" style={{ "--page-motif": motifForArea("clients") } as CSSProperties}><div className="page-hero-content"><p className="eyebrow">CLIENT COMMAND CENTER</p><div className="account-hero-title"><div><h1>{client.name}</h1><p>{[client.billing_address, client.phone, client.email].filter(Boolean).join(" · ") || "Client account"}</p></div><span className="atlas-chip"><Users size={15} />Client account</span></div></div></section>
