...
import {
  acquireMarketingSectionReservation,
  heartbeatMarketingSectionReservation,
  releaseMarketingSectionReservation,
...
export function useSectionReservation(id: string | null) {
  ...
  async function acquire() {
    ...
    try {
      const next = await acquireMarketingSectionReservation(id, {
        signal,
      });
  ...
  async function verify() {
    if (!id) return;
    const signal = abort.current.signal;
    const next = await heartbeatMarketingSectionReservation(id, { signal });
    if (signal.aborted) throw Error("Editor closed");
    setReservation(next);
    if (!next.ownedByCurrentUser)
      throw Error("Another editor holds this section. Your changes are retained.");
  }
  useEffect(() => {
    ...
    return () => {
      controller.abort();
      clearInterval(timer);
      void releaseMarketingSectionReservation(id, { keepalive: true }).catch(
        () => {},
      );
    };
  }, [id]);
