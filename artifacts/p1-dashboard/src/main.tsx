  ...
  async function authSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    await run(async () => {
      ...
      if (authMode === "totp" || authMode === "recovery") {
        const r = authMode === "recovery" ? await auth.twoFactor.verifyBackupCode({code: values.totp}) : await auth.twoFactor.verifyTotp({ code: values.totp });
        if (r.error) throw new Error(r.error.message);
        await session();
        return;
      }
        ...
        <section className="auth-card">
          ...
          <p className="muted">
            {boot && !boot.initialized
              ? boot.configured ? "First-time setup is available to the designated owner." : "Owner setup is awaiting configuration. Account creation is currently unavailable."
          ...
          <form onSubmit={authSubmit}>
            ...
            {["totp","recovery"].includes(authMode) && field("totp", authMode === "recovery" ? "Recovery code" : "Authenticator code")}
            <button className="primary" type="submit">
              Continue <ArrowUpRight size={17} />
            </button>
          </form>
          <div className="auth-links">
            {["totp","recovery"].includes(authMode) && <button onClick={() => setAuthMode(authMode === "totp" ? "recovery" : "totp")}>{authMode === "totp" ? "Use a recovery code" : "Use authenticator"}</button>}
            <button
              onClick={() =>
              ...
              {view === "My Day" && (
                <div className="offline-toolbar">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void run(async () => {
                        await offline.saveDay(person.id, data.work || []);
                        setNotice(
                  ...
                  data.work.map((w: any) => (
                    <article className="work-card" key={w.id}>
                      ...
                      <div className="work-actions">
                        ...
                        {manager && w.status === "reviewed" && !w.published && (
                          <button
                            onClick={() =>
                              void run(async () => {
                                await api(
            ...
            <>
              ...
              {staff && (
                <section className="panel">
                  ...
                  {(data.leads || []).map((lead: any) => <div className="schedule-row" key={lead.id}><div><strong>{lead.name}</strong><small>{lead.location} · {lead.description}</small></div><span className="badge">{lead.status}</span>{!lead.converted_property_id && ["owner","manager","sales"].includes(person.role || "") && <button onClick={() => openForm("convert",lead)}>Convert inquiry</button>}</div>)}
                  {!data.leads?.length && <p className="empty">No inquiries recorded.</p>}
                </section>
              )}
            </>
          )}
            ...
            <section className="panel">
              ...
              {(data.billing || []).map((b: any) => (
                <div className="schedule-row" key={b.id}>
                  ...
                  {staff && b.status === "posted" && !b.ownership_verified && <small role="status">Accounting customer needs reconciliation · hidden from client</small>}
              ...
              {(data["quickbooks/invoices"] || []).map((invoice:any) => <div className="schedule-row" key={"qbo-"+invoice.id}><div><strong>Invoice {invoice.document_number || invoice.id}</strong><small>{invoice.client_name}</small>{staff && !invoice.ownership_verified && <small>Customer mapping needs review · hidden from client</small>}</div><strong>{money(invoice.balance_cents)} outstanding</strong>{invoice.payment_url && invoice.ownership_verified && <a href={invoice.payment_url} target="_blank" rel="noreferrer">Pay invoice</a>}</div>)}
              {!data.billing?.length && !data["quickbooks/invoices"]?.length && (
                <Empty
