...
function EstimateApproval() {
  ...
  return <main className="auth-shell"><section className="auth-card estimate-approval"><h1>Estimate review</h1>{error && <p className="error">{error}</p>}{notice && <p className="notice">{notice}</p>}{!document && !error && !notice && <p>Loading estimate…</p>}{document && <><p><strong>{document.title}</strong></p><p>Estimate revision {document.revision}</p><p>{document.client_name} · {document.property_name}</p>{document.address && <p>{document.address}</p>}<p style={{whiteSpace:"pre-wrap"}}>{document.scope}</p><div className="table-wrap"><table><thead><tr><th>Description</th><th>Quantity</th><th>Rate</th></tr></thead><tbody>{document.line_items.map((item: any) => <tr key={item.position}><td>{item.description}</td><td>{item.quantity} {item.unit || ""}</td><td>{money(item.unit_price_cents)}</td></tr>)}</tbody></table></div><h2>{money(document.amount_cents)}</h2><p>Valid through {formatEstimateExpiry(document.expires_at)}</p>{document.agreement_template_snapshot && <section><h2>Agreement terms</h2><p style={{whiteSpace:"pre-wrap"}}>{document.agreement_template_snapshot}</p></section>}{document.terms && <section><h2>{document.agreement_template_snapshot ? "Additional terms" : "Terms"}</h2><p style={{whiteSpace:"pre-wrap"}}>{document.terms}</p></section>}<p><a href={`/api/public/estimates/${encodeURIComponent(token)}/pdf`}>Download PDF</a></p><div className="heading-actions"><button className="primary" disabled={busy} onClick={() => void decide("approved")}>Approve estimate</button><button disabled={busy} onClick={() => void decide("declined")}>Decline</button></div></>}</section></main>;
}
