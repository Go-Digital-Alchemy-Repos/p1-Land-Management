    ...
    <div className="app">
      ...
      <div className="workspace">
        ...
        <main className="content">
          {!accountWorkspace && view !== "Website Documents" && (routeUnavailable || !["Website Forms", "Media Library", "CMS Pages", "Website Sections", "Website Editor", "Website Galleries", "Website Head Tags", "Website Blog", "Website Menus", "Website Team", "Website Sidebars"].includes(view)) && <div className={view === "Overview" ? "page-heading page-hero" : "page-heading"}>
            <div>
          ...
          {view === "Website Editor" && <Suspense fallback={<p role="status">Loading website editor…</p>}><WebsiteEditor canUseMedia={can("marketing.content.media")} key={`${person.id}:${(person.capabilities || []).join(",")}`}/></Suspense>}
          ...
          {view === "Website Galleries" && <Suspense fallback={<p role="status">Loading galleries…</p>}><GalleryManager canUseMedia={can("marketing.content.media")} key={`${person.id}:${(person.capabilities || []).join(",")}`}/></Suspense>}
          ...
          >
            ...
            {form === "contacts" ? (
              ...
              <form onSubmit={submit}>
                ...
                {form === "activate-recurring" && <><p>{selected?.estimate_allocation_id ? "Choose the default team member. The approved first visit and time are retained." : "Set the first visit and default team member."}</p><label>Default assignee<select name="assignedTo" required><option value="">Choose a team member</option>{(data.staff || []).filter((candidate: any) => candidate.canAssignWork).map((staffMember: any) => <option key={staffMember.id} value={staffMember.id}>{staffMember.name}</option>)}</select></label><label>First visit<input name="nextDate" type="date" required defaultValue={String(selected?.next_date || "").slice(0, 10)} readOnly={Boolean(selected?.estimate_allocation_id)} /></label><label>Default time<input name="localTime" type="time" required defaultValue={String(selected?.local_time || "").slice(0, 5)} readOnly={Boolean(selected?.estimate_allocation_id)} /></label></>}
                {form === "billing" && (
                  <>
                    {propertySelect()}
...
function EstimateApproval() {
  ...
  return <main className="auth-shell"><section className="auth-card estimate-approval"><h1>Estimate review</h1>{error && <p className="error">{error}</p>}{notice && <p className="notice">{notice}</p>}{!document && !error && !notice && <p>Loading estimate…</p>}{document && <><p><strong>{document.title}</strong></p><p>Estimate revision {document.revision}</p><p>{document.client_name} · {document.property_name}</p>{document.address && <p>{document.address}</p>}{document.composition_document ? <><p>Valid through {formatEstimateExpiry(document.expires_at)}</p><ComposedProposal document={document.composition_document} /></> : <><p style={{whiteSpace:"pre-wrap"}}>{document.scope}</p><div className="table-wrap"><table><thead><tr><th>Description</th><th>Quantity</th><th>Rate</th></tr></thead><tbody>{document.line_items.map((item: any) => <tr key={item.position}><td>{item.description}</td><td>{item.quantity} {item.unit || ""}</td><td>{money(item.unit_price_cents)}</td></tr>)}</tbody></table></div><h2>{money(document.amount_cents)}</h2>{document.agreement_template_snapshot && <section><h2>Agreement terms</h2><p style={{whiteSpace:"pre-wrap"}}>{document.agreement_template_snapshot}</p></section>}{document.terms && <section><h2>{document.agreement_template_snapshot ? "Additional terms" : "Terms"}</h2><p style={{whiteSpace:"pre-wrap"}}>{document.terms}</p></section>}</>}{!document.composition_document && <p>Valid through {formatEstimateExpiry(document.expires_at)}</p>}<p><a href={`/api/public/estimates/${encodeURIComponent(token)}/pdf`}>Download PDF</a></p><div className="heading-actions"><button className="primary" disabled={busy} onClick={() => void decide("approved")}>Approve estimate</button><button disabled={busy} onClick={() => void decide("declined")}>Decline</button></div></>}</section></main>;
}
