...
function emptyColumns(keys: string[]): Columns {
  return Object.fromEntries(keys.map((key) => [key, { items: [], cursor: null }]));
}
...
function PipelineCard({
  ...
  return (
    <article className="sales-pipeline-card">
      <h4>{inquiry.name}</h4>
      <p>{inquiry.reported_company_name || inquiry.location || "Location not provided"}</p>
      {inquiry.next_action && <p className="sales-pipeline-next">{inquiry.next_action}</p>}
      <small>
        {inquiry.owner_name || (inquiry.owner_id ? "Previous owner" : "Unassigned")}
      ...
      {open && (
        <div className="sales-pipeline-card-tools" aria-label={`Inquiry tools for ${inquiry.name}`}>
          ...
          {canOnboard && <LeadOnboarding leadId={inquiry.id} onSaved={onChanged} />}
        </div>
      )}
    </article>
  );
...
export function SalesPipelineBoard({
  ...
  const [columns, setColumns] = useState<Columns>(() => emptyColumns(stageKeys));
  ...
  async function load() {
    ...
    try {
      const pages = await Promise.all(
        stageKeys.map((status) =>
          listSalesInquiries(
            ...
            { signal: AbortSignal.any([controller.current!.signal, AbortSignal.timeout(30000)]) },
          ),
        ),
      );
      ...
      if (generation.current === id && (cause as Error).name !== "AbortError")
        setError("Could not load the sales pipeline. Existing cards remain visible; retry to refresh.");
    } finally {
  ...
  async function more(status: string) {
    ...
    try {
      ...
      setColumns((current) => ({
        ...current,
        [status]: {
          items: [...current[status].items, ...page.items.filter((item) => !current[status].items.some((old) => old.id === item.id))],
          cursor: page.nextCursor,
        },
      }));
    } catch {
      setError("Could not load older inquiries for this stage. Retry to keep the current cards.");
    } finally {
      setLoadingMore(null);
    }
  }
  ...
  return (
    <section className="panel sales-pipeline" aria-label="Sales pipeline">
      <div className="panel-heading">
        <div>
          <h2>Pipeline</h2>
          <p>Manage every inquiry in its current sales stage. Changing a stage uses the existing versioned follow-up workflow.</p>
        </div>
        <div className="sales-pipeline-actions">
          <button type="button" onClick={onCreate}>Add inquiry</button>
          <button type="button" disabled={loading} onClick={() => void load()}>{loading ? "Refreshing…" : "Refresh pipeline"}</button>
        </div>
      </div>
      ...
      <div className="sales-pipeline-grid">
        {stages.map((stage) => {
          ...
          return (
            <section className="sales-pipeline-column" key={stage.key} aria-label={`${stage.label} inquiries`}>
              <h3>{stage.label} <span>{column.items.length}</span></h3>
              {column.items.map((inquiry) => (
                <PipelineCard key={inquiry.id} inquiry={inquiry} canOnboard={canOnboard} onChanged={() => void load()} />
              ))}
              {!loading && !column.items.length && <p className="sales-pipeline-empty">No inquiries in this stage.</p>}
              {column.cursor && (
                <button type="button" disabled={loadingMore === stage.key} onClick={() => void more(stage.key)}>
                  {loadingMore === stage.key ? "Loading…" : "Load older inquiries"}
                </button>
              )}
            </section>
          );
