...
app.use((req, res, next) => {
  res.set({
    ...
    "Content-Security-Policy":
      "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' blob: data: https://tiles.openfreemap.org; connect-src 'self' https://tiles.openfreemap.org; frame-src https://www.p1landmanagement.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'",
