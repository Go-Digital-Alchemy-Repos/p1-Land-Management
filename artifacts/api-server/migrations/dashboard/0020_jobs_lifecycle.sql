...
CREATE TABLE service_request_event (
  ...
  event text NOT NULL CHECK(event IN ('created','estimating','sent','declined','converted','closed','cancelled')),
  version integer NOT NULL CHECK(version>0),
  details jsonb NOT NULL DEFAULT '{}',
  created_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX service_request_event_request_created_idx ON service_request_event(request_id,created_at);
