...
`vault-migrations.ts` advances the deployed encrypted workspace schema from v1 to v2 with an explicit forward-only plan. Native image capture now records its temporary normalized cache file in the account/origin-scoped encrypted workspace before attempting BLOB staging. On the next verified crew-vault open, a pending staging record is either recovered into the regular protected photo outbox, cleaned after a completed encrypted commit, or retained with a nonsecret capture ID if the cache file no longer exists. The app reports that last case for office support without treating it as an uploaded photo. `pnpm test:vault-storage` performs a disposable SQLite v1→v2/restart rehearsal for recovery and missing-file accounting; its Expo key/cipher adapters are mocked, so physical-device crash and low-storage acceptance remains required.

`assets/icon.png` is the committed 1024px P1 native icon and splash asset. The in-app Help screen presents local guidance for invitation/password setup, MFA and recovery codes, expired offline sessions, protected pending work and non-reconstructable device keys. It contains no support contact, credential, token, customer or capture data; saved-capture references are nonsecret local identifiers only. Store identifiers, signing, distribution and physical-device review remain owner-controlled release work.

The signed-in shell uses a role-scoped home description and provides validated previous/next workday controls. It reloads the existing schedule contract after choosing the adjacent calendar day, including across daylight-saving boundaries. The API does not support property-filtered scheduling, so property-to-work drilldown remains an explicitly tracked contract and navigation requirement rather than an untrusted client-side filter.
