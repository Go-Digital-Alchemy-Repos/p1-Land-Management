        ...
        <div className="flex items-center gap-4">
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild className="lg:hidden">
              <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl border-2 border-primary/85 bg-background text-secondary shadow-sm transition-all duration-200 hover:bg-primary/5 hover:text-primary active:scale-95">
              ...
              <div className="flex h-full min-h-0 flex-col">
                <div className="border-b border-border/70 px-6 pb-5 pr-20 pt-[max(1.25rem,env(safe-area-inset-top))]">
                  <Link href="/" onClick={() => setIsOpen(false)} className="inline-flex transition-opacity hover:opacity-80">
                    <img src={logo} alt="P1 Land & Property Management" className="h-9 w-auto" />
                  </Link>
                ...
                <nav aria-label="Mobile navigation" className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
                  ...
                  <div className="mt-2 overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
                    <Link href="/" onClick={() => setIsOpen(false)} className="flex min-h-14 items-center gap-3 px-4 text-secondary transition-colors hover:bg-muted/70 active:bg-muted">
                      <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary"><Home className="h-4 w-4" /></span>
                      <span className="flex-1 font-semibold">Home</span><ChevronRight className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
                    </Link>
                    <Link href="/about" onClick={() => setIsOpen(false)} className="flex min-h-14 items-center gap-3 border-t border-border/70 px-4 text-secondary transition-colors hover:bg-muted/70 active:bg-muted">
                      <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-clay/10 text-clay-ink"><UserRound className="h-4 w-4" /></span>
                      <span className="flex-1 font-semibold">About P1</span><ChevronRight className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
                    </Link>
                    <Link href="/contact" onClick={() => setIsOpen(false)} className="flex min-h-14 items-center gap-3 border-t border-border/70 px-4 text-secondary transition-colors hover:bg-muted/70 active:bg-muted">
                      <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-supporting/10 text-supporting"><Mail className="h-4 w-4" /></span>
                      <span className="flex-1 font-semibold">Contact</span><ChevronRight className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
                    </Link>
                  </div>

                  <p className="mt-7 px-1 text-xs font-bold uppercase tracking-[0.16em] text-muted-foreground">Services</p>
                  <Accordion type="single" collapsible className="mt-2 overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
                    <AccordionItem value="services" className="border-0">
                      <AccordionTrigger className="min-h-16 px-4 py-3 text-secondary hover:bg-muted/70 hover:no-underline">
                        <span className="flex min-w-0 items-center gap-3">
                          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><TreePine className="h-4 w-4" /></span>
                          <span className="min-w-0 flex-1"><span className="block font-semibold">Property services</span><span className="block truncate text-xs text-muted-foreground">Land, landscape, drainage & more</span></span>
                        </span>
                      </AccordionTrigger>
                      <AccordionContent className="border-t border-border/70 bg-muted/30 pb-2 pt-2">
                        <div className="px-3">
                          <Link href="/services" onClick={() => setIsOpen(false)} className="flex min-h-11 items-center justify-between rounded-xl px-3 text-sm font-bold text-primary transition-colors hover:bg-background active:bg-background">
                            <span>Browse all services</span><ArrowUpRight className="h-3.5 w-3.5" aria-hidden="true" />
                          </Link>
                          {services.map((s) => (
                            <Link key={s.href} href={s.href} onClick={() => setIsOpen(false)} className="flex min-h-11 items-center justify-between gap-3 rounded-xl px-3 py-2 text-sm font-medium text-secondary/80 transition-colors hover:bg-background hover:text-primary active:bg-background">
                              <span>{s.name}</span><ArrowUpRight className="h-3.5 w-3.5 shrink-0 text-primary" aria-hidden="true" />
                            </Link>
                          ))}
                          <Link href="/service-areas" className="font-bold text-primary mt-2 flex min-h-11 items-center justify-between rounded-xl border-t border-border/70 px-3 pt-3 transition-colors hover:bg-background" onClick={() => setIsOpen(false)}>
                            <span>Service Areas</span><ArrowUpRight className="h-3.5 w-3.5" aria-hidden="true" />
                          </Link>
                        </div>
                      </AccordionContent>
                    </AccordionItem>
                ...
                <div className="border-t border-border/70 bg-background/90 px-5 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-4 backdrop-blur-xl">
                  <a href="tel:7042218928" className="flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-border bg-card px-4 font-bold text-secondary shadow-sm transition-all hover:border-primary/40 hover:text-primary active:scale-[0.98]">
