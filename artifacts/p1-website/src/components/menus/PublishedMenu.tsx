...
function MenuBranch({item, variant, onNavigate, onForm}: Omit<Props, 'items'> & {item: PublicWebsiteMenuItem}) {
  ...
  return <li className="relative min-w-0" onBlur={event => {if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false);}} onKeyDown={event => {if (event.key === 'Escape' && open) {event.preventDefault(); event.stopPropagation(); setOpen(false); toggle.current?.focus();}}}>
    <div className="flex items-center justify-between gap-2">{link}{item.children.length > 0 && <button ref={toggle} type="button" aria-label={`Show links under ${item.label}`} aria-expanded={open} aria-controls={id} className="shrink-0 rounded p-2 hover:bg-primary/10 focus-visible:outline focus-visible:outline-primary" onClick={() => setOpen(value => !value)}><ChevronDown aria-hidden="true" className={`h-4 w-4 transition-transform ${open ? 'rotate-180' : ''}`} /></button>}</div>
