...
import {
  acceptsPreviewParent,
  BUSINESS_CENTER_ORIGIN,
} from "../../config/preview-origins.mjs";
...
export type CmsField = {
  path: string;
  label: string;
  type: "text" | "textarea" | "image" | "imageAlt" | "ctaTarget";
  required: boolean;
  maxLength: number;
};
export type CmsCollection = {
  page: Record<string, { field: CmsField; value: string }>;
  global: Record<string, { field: CmsField; value: string }>;
};
export function routeId(path: string) {
  return path === "/"
    ? "home"
    : path.replace(/^\/+|\/+$/g, "").replace(/[^a-z0-9]+/g, "-");
}
const Context = createContext<{
  snapshot: CmsSnapshot;
  collect?: CmsCollection;
}>({ snapshot: { route: "/", content: {}, global: {} } });
...
export function cmsValue(
  context: ReturnType<typeof useCms>,
  original: string,
  kind: CmsField["type"] = "text",
  global = false,
  explicitKey?: string,
) {
  const fieldKind =
    kind === "imageAlt" ? "alt" : kind === "textarea" ? "text" : kind;
  const derivedKey = fieldId(original, fieldKind);
  const key =
    explicitKey || legacyCmsFieldKey(original, fieldKind) || derivedKey;
  ...
  if (original.trim() && context.collect) {
    ...
    const label =
      explicitKey === "seoTitle"
        ? "SEO title"
        : explicitKey === "seoDescription"
          ? "SEO description"
          : `${kind === "ctaTarget" ? "Link" : kind === "image" ? "Image" : kind === "imageAlt" ? "Image description" : "Text"}: ${original.trim().slice(0, 95)}`;
    group[key] = {
      field: {
        path: key,
        label,
        type: kind,
        required: false,
        maxLength: kind === "textarea" ? 12000 : 2000,
      },
      value: original,
    };
  }
  ...
  return applyKnownPhoneReference(
    original,
    safeValue(value, kind) ? value : original,
    kind,
...
export function CmsProvider({
  ...
  useEffect(() => {
    ...
    const componentKey =
      new URLSearchParams(location.search).get("cmsComponent") ||
      `${id}-content`;
    ...
    const listener = (event: MessageEvent) => {
      ...
      if (
        !acceptsPreviewParent(event.origin, location.origin) ||
        event.source !== window.parent ||
        !data ||
        data.type !== "core-platform:client-site-preview" ||
        data.protocolVersion !== "1.0" ||
        data.clientStackId !== "p1-land-management" ||
        data.routeId !== routeKey ||
      ...
      const clean = Object.fromEntries(
        Object.entries(data.content).filter(
          ([k, v]) =>
      ...
      setPreview({
        ...
        [componentKey === "site-chrome" ? "global" : "content"]: clean,
      });
    };
    window.addEventListener("message", listener);
    for (const target of new Set([location.origin, BUSINESS_CENTER_ORIGIN]))
  ...
  return (
    <Context.Provider value={{ snapshot: active, collect }}>
      {children}
    </Context.Provider>
  );
}
