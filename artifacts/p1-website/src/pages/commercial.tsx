...
const capabilities = [
  ...
  { title: 'Recurring Site Management', text: 'Build an agreed maintenance schedule with inspection priorities, communication and follow-up expectations defined together.', href: '/services/commercial-landscaping', label: 'Recurring grounds management', value: 'recurring_site_management' },
...
function AssessmentForm() {
  ...
  return <form onSubmit={submit} noValidate onFocus={() => { if (!started.current) { started.current = true; trackAcquisition('form_start'); } }} className="space-y-7 rounded-sm border border-slate-300 bg-white p-5 md:p-9" aria-busy={busy}>
    ...
    <p className="text-sm text-slate-600">Your information is used to review and follow up on this inquiry. Staff will coordinate your free site assessment, access requirements, and available appointment times.</p>
  ...
  return <Layout assessmentCta>
    ...
    <section className="relative isolate overflow-hidden bg-secondary text-white">
      ...
      <div className="site-shell py-20 md:py-28"><div className="max-w-4xl">
        ...
        <p className="mt-5 max-w-2xl text-lg leading-relaxed text-slate-200">Nearly 30 years of experience connects commercial grounds management, corrective earthwork, drainage, and property improvements. P1 coordinates priorities and work sequences around your operations, from initial preparation through recurring exterior facility maintenance.</p>
    ...
    <section className="bg-muted py-20"><div className="site-shell"><p className="text-sm font-bold uppercase tracking-widest text-primary">Eight connected capabilities</p><h2 className="mt-4 max-w-3xl text-3xl font-bold md:text-4xl">Start with the whole site. Define the work precisely.</h2><p className="mt-5 max-w-3xl text-muted-foreground">Connect commercial grounds management, drainage, earthwork, and seasonal care in a practical plan. Your assessment identifies immediate priorities, recurring tasks, and the work sequence that fits your operation.</p><div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{capabilities.map((item, index) => <article key={item.value} className="flex flex-col border border-border bg-background p-7"><p className="text-sm font-bold text-primary">0{index + 1}</p><h3 className="mt-4 text-2xl font-bold">{item.title}</h3><p className="my-5 flex-1 leading-relaxed text-muted-foreground">{item.text}</p><Link href={item.href} className="font-bold text-primary underline underline-offset-4">{item.label}</Link></article>)}</div></div></section>
    ...
    <section className="bg-muted py-20"><div className="site-shell"><p className="text-sm font-bold uppercase tracking-widest text-primary">P1 Commercial Site Assessment</p><h2 className="mt-4 max-w-3xl text-3xl font-bold md:text-4xl">A useful starting point before a long-term commitment.</h2><div className="mt-9 grid gap-8 md:grid-cols-3">{[
      ...
      ['2. Confirm the assessment', 'Start with a free site assessment. Our team coordinates access and a property walk around your operating schedule, then identifies priorities for the next stage.'],
    ...
    ].map(([title, text]) => <div key={title}><h3 className="text-xl font-bold">{title}</h3><p className="mt-4 leading-relaxed text-muted-foreground">{text}</p></div>)}</div><p className="mt-8 max-w-3xl text-sm text-muted-foreground">Scheduling, access approvals, procurement documents, specialist responsibilities, and any separate technical studies are agreed before work begins. Security clearance, uptime, and emergency response commitments require a separate written agreement.</p></div></section>
