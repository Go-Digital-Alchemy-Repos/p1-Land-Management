...
const createEnabledContentStore = options => createContentStore({ ...options, overlayEnabled: true });
...
test('the default-disabled overlay serves source defaults without Core or cache reads', async t => {
  const cacheDir = await temp(t);
  await writeFile(path.join(cacheDir, 'home--home-content.json'), JSON.stringify({
    stackId: 'p1-land-management', routeId: 'home', componentKey: 'home-content',
    revision: 9, content: { title: 'Stale published copy' },
  }));
  let calls = 0;
  const store = createContentStore({ manifest, origin: 'https://core.example', cacheDir, overlayEnabled: false,
    fetcher: async () => { calls++; throw new Error('Core must not be called'); } });
  const before = await store.snapshot('/');
  store.invalidate();
  const after = await store.snapshot('/');
  assert.equal(calls, 0);
  assert.deepEqual(after, before);
  assert.equal(after.content.title, 'Original');
  assert.equal(after.global.title, 'Original');
  assert.equal(after.contentOverlayEnabled, false);
});

test('published content preserves revision, defaults, and unknown routes are absent', async () => {
  const store = createEnabledContentStore({ manifest, origin: 'https://core.example', fetcher: async url => reply(url) });
...
test('rejects foreign identity, invalid revision, unknown fields, unsafe images and links, invalid dates', async () => {
  ...
  for (const value of invalid) {
    const store = createEnabledContentStore({ manifest, origin: 'https://core.example', fetcher: async url => reply(url, value) });
...
test('ETag revalidation and invalidation replace published content', async () => {
  ...
  const store = createEnabledContentStore({ manifest, origin: 'https://core.example', fetcher: async (url, options) => {
...
test('last good persisted content survives process restart and backend outage', async t => {
  const cacheDir = await temp(t);
  await createEnabledContentStore({ manifest, origin: 'https://core.example', cacheDir, fetcher: async url => reply(url) }).snapshot('/');
  const offline = createEnabledContentStore({ manifest, origin: 'https://core.example', cacheDir, fetcher: async () => { throw new Error('offline'); } });
...
test('corrupt or foreign persisted records never become public', async t => {
  ...
  const store = createEnabledContentStore({ manifest, cacheDir, origin: 'https://core.example', fetcher: async () => { throw new Error('offline'); } });
...
test('failed cache writes do not roll back freshly accepted content', async t => {
  ...
  const store = createEnabledContentStore({ manifest, origin: 'https://core.example', cacheDir, fetcher: async url => reply(url) });
  assert.equal((await store.snapshot('/')).content.title, 'Published');
...
test('concurrent page requests share in-flight content fetches', async () => {
  let calls = 0;
  const store = createEnabledContentStore({ manifest, origin: 'https://core.example', fetcher: async url => { calls++; await new Promise(resolve => setTimeout(resolve, 10)); return reply(url); } });
...
test('outage and malformed backend responses retain the last accepted snapshot', async () => {
  let failure;
  const store = createEnabledContentStore({ manifest, origin: 'https://core.example', fetcher: async url => {
...
test('publish invalidation during an in-flight fetch forces the next request to revalidate', async () => {
  ...
  const store = createEnabledContentStore({ manifest, origin: 'https://core.example', fetcher: url => {
    calls++;
...
test('unconditional 304 and invalid persisted JSON degrade to seed safely', async t => {
  ...
  const store = createEnabledContentStore({ manifest, cacheDir, origin: 'https://core.example', fetcher: async () => new Response(null, { status: 304 }) });
...
test('all 35 routes stay within the routine Core read budget while publishing refreshes immediately', async t => {
  ...
  const store = createEnabledContentStore({ manifest: full, origin: 'https://core.example', fetcher: async url => {
    calls++;
